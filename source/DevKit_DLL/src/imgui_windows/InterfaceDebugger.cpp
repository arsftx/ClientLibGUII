...
void InterfaceDebugger::Render() {
	...
	int current_size = (g_CurrentIfUnderCursor->GetRuntimeClass()->m_nObjectSize -
		g_CurrentIfUnderCursor->GetParentRuntimeClass()->m_nObjectSize);

	ImGui::Text("@%p", g_CurrentIfUnderCursor);

	ImGui::Text("Class: %s | Size: 0x%X | 0x%X",
		g_CurrentIfUnderCursor->GetRuntimeClass()->m_lpszClassName,
		g_CurrentIfUnderCursor->GetRuntimeClass()->m_nObjectSize,
		current_size);

	ImGui::Text("Class Parent: %s | Size: 0x%X",
		g_CurrentIfUnderCursor->GetParentRuntimeClass()->m_lpszClassName,
		g_CurrentIfUnderCursor->GetParentRuntimeClass()->m_nObjectSize);

	ImGui::Text("Id: %d", g_CurrentIfUnderCursor->UniqueID());

	ImGui::Text("\t X= %4d | Y= %4d | Width= %4d | Height= %4d",
		bounds.pos.x,
		bounds.pos.y,
		bounds.size.width,
		bounds.size.height);
