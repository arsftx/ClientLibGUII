...
void InterfaceTree::RecurseElement(const CGWndBase* element) {
   ...
   //     printf("pCosInfo %p m_selectedPetUniqueId %d\n", pCosInfo, pCosMgr->GetSelectedPetGameId());
   //
   //     //SCOSInfo *pMyTradePet = pCosMgr->GetTradeInfo();
   //     //if (pMyTradePet)
   //     //    printf("pMyTradePet %p GetItemId %d\n", pMyTradePet, pMyTradePet->GetItemId());
   // }
   //
   // printf("g_pCICPlayer %p pCosMgr %p\n", g_pCICPlayer, pCosMgr);

   //return;

	std::n_list<CGWndBase*> pList = element->GetControlList();
	for (std::n_list<CGWndBase*>::const_iterator it = pList.begin();
		it != pList.end();
		++it) {
