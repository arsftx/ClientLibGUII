...
void CIFTargetWindow::OnUpdateIMPL() {
    reinterpret_cast<void(__thiscall *)(CIFTargetWindow *)>(0x00569bb0)(this);

	//m_pCIFBuffViewer = m_IRM.GetResObj<CIFBuffViewer>(GDR_TW_BUFF, 1);
	//m_pGDR_TW_PLAYERWND = m_IRM.GetResObj<CIFTargetWindowPlayer>(GDR_TW_PLAYERWND, 1); // 0x0
	//m_pGDR_TW_JOB_PLAYERWND = m_IRM.GetResObj<CIFTargetWindowJobPlayer>(GDR_TW_JOB_PLAYERWND, 1); // 0x1
	//m_pGDR_TW_SPECIALMOBWND = m_IRM.GetResObj<CIFTargetWindowSpecialMob>(GDR_TW_SPECIALMOBWND, 1); // 0x2
	//m_pGDR_TW_COMMONENEMY = m_IRM.GetResObj<CIFTargetWindowCommonEnemy>(GDR_TW_COMMONENEMY, 1); // 0x4
	
	CGWndBase::wnd_rect bounds = this->m_pGDR_TW_SPECIALMOBWND->GetBounds();
	CGWndBase::wnd_rect boundsplayers = this->m_pGDR_TW_PLAYERWND->GetBounds();
	CGWndBase::wnd_rect boundsjobplayers = this->m_pGDR_TW_JOB_PLAYERWND->GetBounds();
	CGWndBase::wnd_rect boundscommonenemy = this->m_pGDR_TW_COMMONENEMY->GetBounds();

	if (this->m_pGDR_TW_SPECIALMOBWND->IsVisible()) {
		
		wnd_rect nbounds = this->m_pCIFBuffViewer->GetBounds();
		
		if (bounds.size.height == 78)
		{
			this->m_pCIFBuffViewer->MoveGWnd(nbounds.pos.x, 85);
		}
		if (bounds.size.height == 55)
		{
			this->m_pCIFBuffViewer->MoveGWnd(nbounds.pos.x, 58);
		}
	}
	
	if (this->m_pGDR_TW_PLAYERWND->IsVisible()) {

		wnd_rect nbounds = this->m_pCIFBuffViewer->GetBounds();
		if (boundsplayers.size.height == 36)
		{
			this->m_pCIFBuffViewer->MoveGWnd(nbounds.pos.x, 68);
			
		}
		
#if 0
		printf("boundsplayers X= %4d | Y= %4d | Width= %4d | Height= %4d\n",
			boundsplayers.pos.x,
			boundsplayers.pos.y,
			boundsplayers.size.width,
			boundsplayers.size.height);
#endif
	}
	if (m_pGDR_TW_JOB_PLAYERWND->IsVisible()) {

		wnd_rect nbounds = m_pCIFBuffViewer->GetBounds();
		if (boundsjobplayers.size.height == 36)
		{
			m_pCIFBuffViewer->MoveGWnd(nbounds.pos.x, 68);
		}
	}
	if (m_pGDR_TW_COMMONENEMY->IsVisible()) {

		wnd_rect nbounds = m_pCIFBuffViewer->GetBounds();
		if (boundscommonenemy.size.height == 36)
		{
			m_pCIFBuffViewer->MoveGWnd(nbounds.pos.x, 68);
		}
	}
